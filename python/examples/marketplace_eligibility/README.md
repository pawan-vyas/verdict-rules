...
```bash
# From python/
uv run python examples/marketplace_eligibility/marketplace_eligibility.py
```
...
```bash
# From python/
uv run pytest examples/marketplace_eligibility/
```
...
| `marketplace_eligibility.py` | The real implementation, plus a runnable `__main__` demo. |
| `test_marketplace_eligibility.py` | The full test suite, loading all three JSON files and asserting generically. |
