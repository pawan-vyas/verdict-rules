...
| [009](009-annotated-tag-sha-never-equals-commit-sha.md) | `release-github.yml`'s tag-idempotency check compared an annotated tag's own SHA against a commit SHA — never equal, so it rejected a tag pointing at the exact right commit | Dereference with `^{commit}` before comparing |
