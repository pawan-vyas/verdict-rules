...
| The three rule shapes | The private `_writtenPredicate`/`_practicalPredicate`/`_exemptionPredicate` helpers compose into whichever shape each `subjectType`'s own builder returns |
| Dispatch by `subjectType` | `_subjectRuleBuilders`, a map of one builder per type — `ruleForSubject()` itself is a lookup, never a branch |
...
`_subjectRuleBuilders`, a real code change, since a new *kind* of pass
